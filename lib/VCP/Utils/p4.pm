...
=head1 P4::Client Support (Experimental)

If the P4::Client module is installed, it can be used to connect to
the p4d instead of the command line client.  While significantly faster,
this mode is not ready for prime time due to unresolved issues with
the underlying C++ API (it's been a while since I tested it, but
as I recall, there was a reply buffer sequencing problem and a request
...
use constant use_p4_api_if_present => $ENV{VCPP4API};
...
sub p4_clients {
   ...
   $self->p4( [ "clients", ], undef, \my $clients ) ;
   return () if empty $clients;
